...
from snowman import get_snowman_game, set_snowman_bot  # 눈사람 게임 추가
...
class MinigamesCog(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.snowman_game = get_snowman_game()  # 눈사람 게임 추가
        ...
        if game_type in ["all", "snowman"] and channel_id in self.snowman_game.active_games:
            ...
            try:
                ...
                if "move_task" in game_data and game_data["move_task"]:
                    ...
                    try:
                        await game_data["move_task"]
                    except asyncio.CancelledError:
                        pass
                
                # 리액션 리스너 태스크 정리
                if "reaction_task" in game_data and game_data["reaction_task"]:
                    game_data["reaction_task"].cancel()
                    try:
                        await game_data["reaction_task"]
                    except asyncio.CancelledError:
                        pass
            except Exception as e:
                logger.error(f"Error cleaning up snowman tasks: {e}")
